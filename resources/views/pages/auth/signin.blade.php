    ...
    <div class="flex flex-col  flex-1 w-96">
        ...
        <div class="flex flex-col justify-center flex-1 w-full max-w-md">
            <div x-data="{ showUsername: false }">
                <div class=" mb-3 justify-center sm:mb-8">

                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
                        <h1 class="py-5 text-2xl font-semibold text-gray-800 dark:text-gray-100 transition-all duration-300 font-[Poppins] text-start"
                ...
                @section('formLogin')
                    @foreach ($formdata as $index => $form)
                        @foreach ($form as $field)
                            @if ($index === 'userAuth')
                                @if ($currentRouteName == 'login' && in_array($field['name'], ['username', 'email', 'password']))
                                    <!-- {{ $field['title'] }} -->
                                    <div>
