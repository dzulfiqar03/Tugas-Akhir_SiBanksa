      ...
      <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        >
          ...
          <template v-if="show === false">
            ...
            <div
              v-if="$page.props.auth?.user?.user_detail?.status === 'Pengajuan Verifikasi'"
...
>
  <div class="flex justify-around items-center h-16">
     ...
     <button
      @click="$inertia.get('/Warga/dashboard')"
      class="flex flex-col items-center text-xs  transition"
      :class="page.url.startsWith('/Warga/dashboard')
        ? 'text-emerald-600  '
    ...
    <!-- Tracking -->
    <button
      @click="$inertia.get('/Warga/tracking')"
      class="flex flex-col items-center text-xs p-2 transition"
      :class="page.url.startsWith('/Warga/tracking')
        ? 'text-emerald-600 '
        : 'text-gray-500 dark:text-gray-400'"
    >
      <i class="fas fa-route text-lg mb-1"></i>
    ...
    <!-- Transaksi -->
    <button
      @click="$inertia.get('/Warga/transaksi')"
      class="flex flex-col items-center text-xs p-2 transition"
      :class="page.url.startsWith('/Warga/transaksi')
                ? 'text-emerald-600  '
        : 'text-gray-500 dark:text-gray-400'"
    >
    ...
    <button
      ...
      :class="page.url.startsWith('/profile')
        ...
        : 'text-gray-500 dark:text-gray-400'"
    >
      <i class="fas fa-user text-lg mb-1"
      ...
      <!-- DARK MODE BUTTON -->
      <div
        class="fixed lg:flex hidden z-50 right-6"
        :class="isWarga ? 'bottom-24 md:bottom-6' : 'bottom-6'"
      >
        <button
          @click="toggleTheme"
