    ...
    buttons: [
        {
            ...
            pageSize: 'A4',
            title: 'Laporan Jadwal Pelaksanaan SiBanksa RT' + (page.props.auth.user.user_detail?.id_rt || '-') + ' Tanggal ' + new Date().toLocaleDateString('id-ID').replace(/\//g, '-'),
                ...
                config.customize = function (doc) {
                    ...
                    doc.content.splice(0, 1, {
                        columns: [
                            ...
                            { stack: [{ text: 'LAPORAN JADWAL', fontSize: 16, bold: true, alignment: 'right' }, { text: `UNIT RT-0${userDetail?.id_rt || '-'}`, fontSize: 10, alignment: 'right', color: '#9ca3af' }], width: '*' }
                        ]
                    ...
                    doc.content.push({ text: '\n\n' }, {
                        columns: [{ text: '', width: '*' }, {
                            width: 180, stack: [
                                ...
                                { text: `(Ketua RW)`, alignment: 'center', bold: true },
...

        {
            ...
            title: 'Laporan Jadwal Pelaksanaan SiBanksa RT-0' + (page.props.auth.user.user_detail?.id_rt || '-') + ' Tanggal ' + new Date().toLocaleDateString('id-ID').replace(/\//g, '-'),
        ...
        },
        {
            ...
            action: async function (e, dt, button, config) {
                ...
                config.customize = function (win) {
                    ...
                    const tableRows = props.jadwal.user_detail.jadwal?.map((item, index) => `
        ...
        <div style="padding: 40px; border-top: 10px solid #10b981; background: white;">
           ...
           <div style="display: flex; justify-content: flex-end; margin-top: 40px;">
                            <div style="text-align: center; width: 220px;">
                                ...
                                <p style="font-weight: bold; font-size: 12px; text-transform: uppercase;">(Ketua RW)</p>
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex items-center gap-2">
                            <p class="text-black dark:text-gray-300">
                                {{ jadwal.user_detail.telephone_number !== null ? (showNumber ?
                                    jadwal.user_detail.telephone_number : maskPhone(jadwal.user_detail.telephone_number)) ||
                                    'Belum diisi' : 'Belum diisi' }}
                            </p>


                            <button v-if="jadwal.user_detail.telephone_number !== null" type="button"
                                @click="showNumber = !showNumber" class="">
                ...
                <DataTable ref="dtInstance" :options="dtOptions" class="w-full display stripe hover cell-border">
                    ...
                    <tbody class="divide-y dark:divide-gray-700 font-medium">
                        <tr v-for="(value, index) in jadwal.user_detail.jadwal" :key="index"
                            ...
                            <td class="px-6 py-4 text-black dark:text-gray-300">{{ value.tanggal_setoran ? new
                                            Date(value.tanggal_setoran).toLocaleDateString('id-ID', {
                                                day:
                                        'numeric', month: 'long', year: 'numeric' }) : '-' }}</td>
