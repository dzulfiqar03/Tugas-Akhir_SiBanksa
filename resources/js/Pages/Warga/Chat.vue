  ...
  <AuthenticatedLayout :sidebardata="sidebardata" :breadcrumb-items="breadcrumbItems">
    <div class="flex h-full bg-gray-100 dark:bg-gray-950 overflow-hidden relative">
      ...
      <div :class="[
        ...
        <div class="p-4 bg-white dark:bg-gray-900 border-b border-white  dark:border-gray-800 flex justify-between items-center sticky top-0">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-emerald-600 rounded-full flex items-center justify-center text-white font-bold ring-2 ring-emerald-100 dark:ring-emerald-900">
              {{ $page.props.auth.user.user_detail.fullName?.charAt(0) }}
            </div>
            <h1 class="font-bold text-gray-800 dark:text-gray-100 text-lg">Pesan</h1>
          </div>
          <button @click="tambahOrang" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <div v-for="chat in filteredChatList" :key="chat.id" 
            ...
            <div class="relative">
              ...
              <div v-if="chat.online === 'Online'" class="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-white dark:border-gray-900 rounded-full"></div>
            </div>

            <div class="flex-1 min-w-0">
              <div class="flex justify-between items-baseline mb-1">
                <span class="font-bold text-gray-900 dark:text-gray-100 truncate">{{ chat.fullName }}</span>
                <span class="text-[10px] text-gray-400 dark:text-gray-500 uppercase">{{ chat.user_chat.at(-1)?.time }}</span>
              </div>
              <div class="flex justify-between items-center">
                <p class="text-xs text-gray-500 dark:text-gray-400 truncate pr-4">
                  {{ chat.user_chat.at(-1)?.message || 'Belum ada pesan' }}
                </p>
                <span v-if="chat.countUnreadMessage > 0" class="bg-emerald-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-black">
                  {{ chat.countUnreadMessage }}
                </span>
              </div>
            </div>

            <div v-if="idBtn === chat.id" class="absolute right-4 top-12 w-48 bg-white dark:bg-gray-800 shadow-2xl rounded-xl border border-gray-400 dark:border-gray-700 z-50 py-2">
      ...
      <div v-if="activeChat?.id" :class="[
        ...
        <div class="p-3 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-white dark:border-gray-800 flex items-center gap-3 sticky top-0 z-20 shadow-sm">
          ...
          <div @click="isProfileChatOpen = true" class="w-10 h-10 bg-emerald-600 rounded-full flex items-center justify-center text-white font-bold cursor-pointer">
            {{ activeChat.fullName.charAt(0) }}
          </div>
          <div class="flex-1">
            <div class="font-bold text-gray-800 dark:text-gray-100 leading-tight">{{ activeChat.fullName }}</div>
            <div class="text-[10px]" :class="activeChat.online === 'Online' ? 'text-emerald-500 font-bold' : 'text-gray-400 dark:text-gray-500'">
        ...
        <div ref="chatBody" class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          <div v-for="(msg, i) in activeChat.user_chat" :key="i"
            ...
            <div @contextmenu.prevent="idBtn = msg.id"
              ...
              <div class="text-[9px] text-right text-gray-500 dark:text-gray-400 mt-1 uppercase">{{ msg.time }}</div>

              <div v-if="idBtn === msg.id" :class="[
                'absolute top-full mt-2 w-40 bg-white dark:bg-gray-800 shadow-2xl rounded-xl border border-gray-400 dark:border-gray-700 z-50 overflow-hidden',
        ...
        <div class="p-4 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-t border-gray-300 dark:border-gray-800">
          <div class="flex gap-2 items-center bg-gray-100 dark:bg-gray-800 rounded-2xl px-4 py-1">
            <input v-model="newMessage" @keyup.enter="sendMessage" 
                   ...
                   class="flex-1 bg-transparent border-none text-black focus:ring-0 text-sm py-2 dark:text-gray-100 dark:placeholder-gray-500" />
      ...
      <div v-else class="hidden md:flex flex-1 items-center justify-center bg-gray-50 dark:bg-gray-950">
        <div class="text-center">
          <div class="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-emerald-500">
             <i class="fas fa-comments text-3xl"></i>
          </div>
          <h2 class="text-lg font-bold text-gray-700 dark:text-gray-200">Pilih Percakapan</h2>
          <p class="text-sm text-gray-400 dark:text-gray-500">Klik pada salah satu chat untuk memulai pesan</p>
        </div>
      </div>
      ...
      <div v-if="isProfileChatOpen" :class="[
        ...
        <div class="text-center py-5 flex flex-col space-y-3">
          <div class="w-24 h-24 m-auto bg-emerald-600 rounded-full flex items-center justify-center text-white font-bold text-2xl">
            {{ activeChat.fullName.charAt(0) }}
          </div>
          <div class="flex-1 flex-col text-black dark:text-gray-100 space-y-4">
            <div class="font-bold text-gray-800 dark:text-gray-100 leading-tight text-xl">{{ activeChat.fullName }}</div>

            <div class="flex bg-white dark:bg-gray-900 w-max m-auto p-5 rounded-xl shadow dark:shadow-gray-950 items-center justify-center space-x-3">
              <div class="text-md" :class="activeChat.online === 'Online' ? 'text-emerald-500 font-bold' : 'text-gray-400'">
                {{ activeChat.online === 'Online' ? '• Online' : 'Offline' }}
              </div>
            ...
            <div class="flex flex-col m-auto p-7 justify-between items-start space-y-3">
              <div class="text-md text-gray-800 dark:text-gray-200 flex p-5 w-full bg-white dark:bg-gray-900 rounded-xl shadow dark:shadow-gray-950 justify-between">
                <span><i class="fas fa-image mr-2"></i> Media and Docs</span>
                <span> {{ activeChat.documentCount + activeChat.imageCount }}</span>
              </div>

              <div class="w-full">
                <div class="text-md flex p-5 w-full bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 rounded-t-xl shadow dark:shadow-gray-950 justify-between border-b border-white dark:border-gray-800">
                  <h1>Mute Sound</h1>
                ...
                <div class="text-md flex p-5 w-full bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 rounded-b-xl shadow dark:shadow-gray-950 justify-between">
                  <h1>Dark Mode</h1>
              ...
              <div class="w-full">
                <div class="bg-white dark:bg-gray-900 rounded-t-xl shadow dark:shadow-gray-950 py-2 border-b border-white dark:border-gray-800">
                  <button @click.stop="deleteMessage(chat.id)" class="w-full text-left px-4 py-2 text-md text-red-600 hover:text-black dark:hover:text-white font-bold">
                    Hapus Percakapan
                  </button>
                </div>
                <div class="bg-white dark:bg-gray-900 rounded-b-xl shadow dark:shadow-gray-950 py-2">
                  <button type="button" @click="sendLogout" class="w-full flex items-center gap-3 px-4 py-2 text-md bg-white dark:bg-gray-900 font-bold text-red-600 hover:text-black dark:hover:text-white">
...
const sendMessage = () => {
  ...
  router.post(
    route('warga.add-chat', activeChat.value.id), 
    { message: newMessage.value },
    {
      preserveScroll: true,
      onSuccess: () => {
