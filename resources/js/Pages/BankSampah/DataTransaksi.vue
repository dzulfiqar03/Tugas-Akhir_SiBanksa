...
exportOptions: {
    columns: [0, 1, 2, 3, 4, 5],
        // Membersihkan HTML agar hanya teks yang diambil
        format: {
            body: function (data, row, column, node) {
                // Jika kolom adalah kolom nomor rekening (indeks ke-2)
                if (column === 2) {
                    // Ambil teks dari elemen span/div, abaikan script masking
                    return node.querySelector('span') ? node.querySelector('span').getAttribute('data-full') : data;
                }
                // Hapus tag HTML di kolom lain
