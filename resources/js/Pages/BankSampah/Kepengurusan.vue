    ...
    buttons: [
        {
            ...
            pageSize: 'A4',
            title: 'Laporan Kepengurusan SiBanksa RT' + (page.props.auth.user.user_detail?.id_rt || '-') + ' Tanggal ' + new Date().toLocaleDateString('id-ID').replace(/\//g, '-'),
                ...
                config.customize = function (doc) {
                    ...
                    doc.content.splice(0, 1, {
                        columns: [
                            ...
                            { stack: [{ text: 'LAPORAN KEPENGURUSAN', fontSize: 16, bold: true, alignment: 'right' }, { text: `UNIT RT-0${userDetail?.id_rt || '-'}`, fontSize: 10, alignment: 'right', color: '#9ca3af' }], width: '*' }
                        ]
...

        {
            ...
            title: 'Laporan Kepengurusan SiBanksa RT-0' + (page.props.auth.user.user_detail?.id_rt || '-') + ' Tanggal ' + new Date().toLocaleDateString('id-ID').replace(/\//g, '-'),
        ...
        },
        {
            ...
            action: async function (e, dt, button, config) {
                ...
                config.customize = function (win) {
                    ...
                    const tableRows = props.kepengurusan?.map((item, index) => `
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 40px;">
                <thead>
                    <tr style="background: #f9fafb; color: #6b7280; font-size: 10px; text-transform: uppercase; letter-spacing: 1px;">
                        ...
                        <th style="padding: 12px; border-bottom: 2px solid #f3f4f6; text-align: left;">Divisi</th>
                    </tr>
                </thead>
                <tbody>
                    ${tableRows}
        ...
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button @click="openCreateForm"
                    ...
                    {{ showForm ? 'Tutup Form' : 'Tambah Kepengurusan' }}
                </button>
            </div>
