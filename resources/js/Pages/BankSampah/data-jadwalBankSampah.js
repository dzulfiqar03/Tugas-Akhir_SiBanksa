$(document).ready(function () {
    ...
    let table = new DataTable('#data-jadwalBankSampah', {
        pageLength: 5,
        responsive: true,
                ...
                buttons: [
                    {
                        ...
                        title: 'Data Jadwal Pelaksanaan Bank Sampah',
                        exportOptions: {
                        ...
                        customize: function (doc) {
                            ...
                            doc.content.splice(0, 0, {
                                columns: [
                                    ...
                                    {
                                        text: 'Bank Sampah - Data Jadwal Pelaksanaan Bank Sampah',
                                        alignment: 'right',
                                        fontSize: 16,
                                        bold: true,
                                        margin: [0, 20, 0, 0]
...
<div class="lg:flex justify-end hidden items-center gap-4">
    ...
    $(document).on('click', '.btn-tool', function () {
        ...
        $('#formJadwalPelaksanaan input[name="_method"]').val("POST");
        $('#formJadwalPelaksanaan input[name="id"]').val("");
        $('#formJadwalPelaksanaan input[name="nama_sampah"]').val("");
        $('#formJadwalPelaksanaan input[name="satuan"]').val("");
        $('#formJadwalPelaksanaan input[name="harga"]').val("");
        $('#formJadwalPelaksanaan input[name="kategori"]').val("");
    });
    ...
    //     $('#formJadwalPelaksanaan').on('submit', function(e) {
    //     e.preventDefault();

    //     // reset error
    //     $('#formJadwalPelaksanaan input, #formJadwalPelaksanaan select').removeClass('border-red-500 ring-1 ring-red-500');
    //     $('#error-message').fadeOut();

    //     const method = $('#formJadwalPelaksanaan input[name="_method"]').val();
    ...
    $('#formJadwalPelaksanaan').on('submit', function (e) {
        ...
        const method = $('#formJadwalPelaksanaan input[name="_method"]').val();
        const id = $('#formJadwalPelaksanaan input[name="id"]').val();


        const url = method === 'POST' ?
            `/bank-sampah/Jadwal/Create` :
            `/bank-sampah/Jadwal/Update/${id}`;
    ...
    $(document).on('click', '#data-jadwalBankSampah .btn-edit', function () {
        const data = $(this).data();

        // isi form
        $('#formJadwalPelaksanaan input[name=_method]').val("PUT");
        $('#formJadwalPelaksanaan input[name=id]').val(data.id);
        $('#formJadwalPelaksanaan input[name=id_userdetail]').val(data.id_userdetail);
        var tanggalHanya = data.tanggal_setoran.split(' ')[0];

        $('#formJadwalPelaksanaan input[name="tanggal_setoran"]').val(tanggalHanya);
        ...
        if (isCollapsed) {
            ...
            $card.find('.btn-simpan').text('Update Jadwal')
        } else {
            $card.addClass('collapsed-card');
            $card.find('.card-body').slideUp(200);
            $card.find('.btn-cancel').addClass('hidden');
    ...
    $(document).on('click', '.btn-delete', function () {
        ...
        const post = () => {
            $.ajax({
                url: `/bank-sampah/Jadwal/Delete/${id}`,
                type: 'POST',
                data: {
                    _method: 'DELETE',
                    _token: token
