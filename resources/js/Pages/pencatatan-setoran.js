$(document).ready(function () {
    ...
    let table = new DataTable('#dataSetoran', {
        pageLength: 5,
        responsive: true,
        lengthMenu: [5, 10, 25, 50],
         columnDefs: [
        {
            targets: 2,          // kolom kategori
            visible: false,      // disembunyikan
            searchable: true     // tapi tetap bisa difilter
        },
        {
            targets: 'no-print', // kolom aksi
            searchable: false,
            orderable: false
        }
    ],
...
<div class="lg:flex justify-end hidden items-center gap-4">
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        let selected = $('#kategoriFilter').val();          // value dari filter
        let kategori = $(table.row(dataIndex).node()).data('kategori');

        // Tampilkan semua jika "Semua"
        if (selected === "" || selected === kategori) {
            return true;
        }
        return false;
    });
    ...
    $(document).on('click', '.btn-tool', function () {
        ...
        $('#form_book input[name="fullName"]').val("");
        $('#form_book input[name="address"]').val("");
    ...
    $(document).on('click', '#dataNasabah .btn-edit', function () {
        ...
        $('#form_book input[name=fullName]').val(data.name);
        $('#form_book input[name=address]').val(data.address);
        $('#form_book select[name=rt]').val(data.rt);
        $('#form_book select[name=status]').val(data.status);
        $('#form_book input[name=urlProfil]').val(data.urlProfil);
    });
