...
const filteredUsers = computed(() => {
    return props.nasabah.filter(user => {
...
const listRT = computed(() => [...new Set(props.nasabah.map(u => u.user_detail?.id_rt))].sort((a, b) => a - b));
const listRoles = computed(() => [...new Set(props.nasabah.map(u => u.user_detail?.roles?.role))]);
...
const dtOptions = {
    ...
    columns: [
        ...
        { data: 'user_detail.roles.role' },
        { data: 'user_detail.telephone_number' },
        { data: 'user_detail.pencairan_via' },
        { data: 'user_detail.status' },
        { data: 'is_online',
          render: (data) => data ? '<span class="px-3 py-1 rounded-full bg-emerald-500 animate-pulse">Online</span>' : '<span class="px-3 py-1 rounded-full bg-gray-500 text-white">Offline</span>'
        },
        {
            data: null,
            defaultContent: '<button class="view-btn"><i class="fas fa-arrow-right"></i></button>',
            orderable: false
        }
    ],
     language: {
...
const viewDetail = (id) => {
    router.get(route('developer.show-user', id));
};
