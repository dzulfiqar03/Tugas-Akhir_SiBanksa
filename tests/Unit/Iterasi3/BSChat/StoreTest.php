 ...
 */
function kirimPesanChatBS(UserDetail $pengirimDetail, int|string $targetId, string $message, string $name)
{
    return test()->actingAs($pengirimDetail->user)
        ->post(route('bs.add-chat', $targetId), [
            'message' => $message,
            'name'    => $name,
        ]);
}
...
test('UT.BS.CHAT.001 - penerima pesan bukan AI Banksa, chat tersimpan dan notifikasi terkirim', function () {
    ...
    $pengirim = buatUserChatBS(3, 'pengirim', 'Pengirim Warga');
    $target   = buatUserChatBS(3, 'penerima', 'Penerima Warga');

    kirimPesanChatBS($pengirim, $target->user->id, 'Halo, apa kabar?', 'Penerima Warga')
        ->assertRedirect();
 ...
 * ========================================================= */
test('UT.BS.CHAT.002 - pesan ke AI Banksa tanpa kata kunci', function () {
    $user = buatUserChatBS(3, 'warga1', 'Warga Satu');

    kirimPesanChatBotBS($user, $user->user->id, 'Selamat pagi semuanya');
 ...
 * ========================================================= */
test('UT.BS.CHAT.003 - kata kunci rekening', function () {
    $user = buatUserChatBS(3, 'warga2', 'Warga Dua');

    kirimPesanChatBotBS($user, $user->user->id, 'Berapa nomor rekening 1234567890 saya?');
 ...
 * ========================================================= */
test('UT.BS.CHAT.004 - kata kunci setoran, role Petugas Bank Sampah', function () {
    $petugas = buatUserChatBS(2, 'petugas1', 'Petugas Bank Sampah');
    ...
    kirimPesanChatBotBS($petugas, $petugas->user->id, 'Berapa total setoran saya?');
 ...
 *  -> Bot menampilkan total setoran nasabah bersangkutan
 * ========================================================= */
test('UT.BS.CHAT.005 - kata kunci setoran, role Warga atau Nasabah', function () {
    $warga = buatUserChatBS(3, 'warga3', 'Warga Tiga');
    ...
    kirimPesanChatBotBS($warga, $warga->user->id, 'Cek setoran saya dong');
 ...
 * ========================================================= */
test('UT.BS.CHAT.006 - kombinasi bulan ini atau sekarang adalah dead code', function () {
    $warga = buatUserChatBS(3, 'warga4', 'Warga Empat');
    ...
    kirimPesanChatBotBS($warga, $warga->user->id, 'Setoran bulan ini berapa ya?');
 ...
 * ========================================================= */
test('UT.BS.CHAT.007 - kata kunci jumlah dan rw', function () {
    $user = buatUserChatBS(3, 'warga5', 'Warga Lima');

    Sampah::factory()->count(4)->create([
        'id_userdetail' => $user->id,
        'nama_sampah'   => 'Plastik',
        'harga'         => 1000,
        'satuan'        => 'kg',
    ...
    kirimPesanChatBotBS($user, $user->user->id, 'Jumlah sampah di RW berapa?');
    ...
    expect($chat->bot_response)
        ->toContain('Jumlah Jenis Sampah di RW anda ada')
        ->toContain('4');
});
 ...
 * ========================================================= */
test('UT.BS.CHAT.008 - kata kunci jumlah dan sampah', function () {
    $user = buatUserChatBS(3, 'warga6', 'Warga Enam');
    ...
    kirimPesanChatBotBS($user, $user->user->id, 'Jumlah jenis sampah saya ada berapa?');
 ...
 * ========================================================= */
test('UT.BS.CHAT.009 - kata kunci jumlah tanpa sub kata kunci', function () {
    $user = buatUserChatBS(3, 'warga7', 'Warga Tujuh');

    kirimPesanChatBotBS($user, $user->user->id, 'Berapa jumlah nya ya?');
 ...
 *  -> Bot merespon "Maaf saya tidak bisa memahami anda"
 * ========================================================= */
test('UT.BS.CHAT.010 - kata kunci di luar cakupan rekening, setoran, jumlah', function () {
    $user = buatUserChatBS(3, 'warga8', 'Warga Delapan');

    // "Nasabah" ada di daftar keyword tapi tidak masuk kondisi
    // rekening/setoran/jumlah, sehingga jatuh ke else terakhir.
    kirimPesanChatBotBS($user, $user->user->id, 'Saya nasabah tertinggi ya?');
