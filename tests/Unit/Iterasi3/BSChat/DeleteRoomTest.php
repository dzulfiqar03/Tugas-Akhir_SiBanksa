...
function deleteRoomChatBS(UserDetail $user, int|string $roomId)
{
    return test()->actingAs($user->user)
        ->delete(route('bs.delete-roomChat', $roomId));
}
/**
...
test('UT.BS.DELCHAT.001 - delete room chat berhasil', function () {

    $user = buatUserChatBS(3, 'hapus1', 'User Hapus');
...
test('UT.BS.DELCHAT.002 - delete room chat gagal', function () {

    $user = buatUserChatBS(3, 'hapus2', 'User Hapus');
