...
test('UT.TRSC.STORE.001 - Menyimpan Bukti Pencairan dengan inputan valid', function () {
    ...
    $response = $this->post(route('bs.add-transaction', $this->setoran->id), $payload);
...
test('UT.TRSC.STORE.002 - Menyimpan Bukti Pencairan dengan inputan error', function () {
    $this->actingAs($this->currentUser);

    $this->mock(TransactionServices::class, function ($mock) {
    ...
    $response = $this->post(route('bs.add-transaction'), $payload);
