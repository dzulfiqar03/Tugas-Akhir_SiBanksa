...
function deleteRoomChatWW(UserDetail $user, int|string $roomId)
{
    return test()->actingAs($user->user)
        ->delete(route('warga.delete-roomChat', $roomId));
}
/**
...
test('UT.WW.DELCHAT.001 - delete room chat berhasil', function () {

    $user = buatUserChatWW(3, 'hapus1', 'User Hapus');
...
test('UT.WW.DELCHAT.002 - delete room chat gagal', function () {

    $user = buatUserChatWW(1, 'hapus2', 'User Hapus');
