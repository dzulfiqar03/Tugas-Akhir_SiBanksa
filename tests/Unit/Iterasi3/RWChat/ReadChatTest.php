...
function readChatRW(UserDetail $user, int|string $targetId, string $message = '')
{
    return test()->actingAs($user->user)
        ->put(route('rw.read-chat', $targetId), [
            'message' => $message,
        ]);
}
...
test('UT.RW.READCHAT.001 - target user tidak ditemukan', function () {

    $pengirim = buatUserChatRW(1, 'reader1', 'Reader Satu');
...
test('UT.RW.READCHAT.002 - read chat berhasil', function () {

    $pengirim = buatUserChatRW(1, 'reader2', 'Reader Dua');
    $penerima = buatUserChatRW(1, 'reader3', 'Reader Tiga');
...
test('UT.RW.READCHAT.003 - read chat gagal', function () {

    $pengirim = buatUserChatRW(1, 'reader4', 'Reader Empat');
    $penerima = buatUserChatRW(1, 'reader5', 'Reader Lima');
